...
func _init() -> void:
    ...
    if target == null:
        return
    if not (target is WorldEnvironment):
        printerr("Target node must be a WorldEnvironment: %s" % _scene_relative_path(root, target))
        quit(1)
        return
    ...
    var create_environment_if_missing := _parse_bool(config.get("create_environment_if_missing", true), true)
    ...
    if environment_parameters.is_empty() and node_parameters.is_empty():
        printerr("Provide at least one of `environment_parameters` or `node_parameters`.")
        quit(1)
        return

    var world_environment: WorldEnvironment = target
    var environment_created := false
    if world_environment.environment == null:
        if not create_environment_if_missing and not environment_parameters.is_empty():
            printerr("Target WorldEnvironment has no Environment resource and auto-create is disabled.")
            quit(1)
            return
    ...
    var supported_environment_parameters := []
    if world_environment.environment != null:
        supported_environment_parameters = _supported_parameters(world_environment.environment, [])
    ...
    var updated_environment_parameters := []
    if not environment_parameters.is_empty():
        if world_environment.environment == null:
            printerr("Target WorldEnvironment does not have an Environment resource.")
            quit(1)
            return
        updated_environment_parameters = _apply_property_updates(
    ...
    print(JSON.stringify({
        ...
        "node_name": str(world_environment.name),
